...
use rustc_middle::mir::{Body, Operand, Rvalue, StatementKind, TerminatorKind, UnwindAction};
            ...
            for (bb, data) in body.basic_blocks.iter_enumerated() {
                ...
                match &terminator.kind {
                    ...
                    TerminatorKind::SwitchInt { targets, discr, .. } => {
                        ...
                        let type_str = if targets.iter().count() == 1 {
                            "IF"
                        } else {
                            "SWITCH"
                        };
                    ...
                    TerminatorKind::Call {
                        ...
                        if let ty::FnDef(func_def_id, _) = func_ty.kind() {
                            ...
                            if target_method_id.is_some() || is_ffi {
                                ...
                                for arg in args {
                                    let arg_id =
                                        process_operand(&arg.node, graph, body, tcx, &last_def);
