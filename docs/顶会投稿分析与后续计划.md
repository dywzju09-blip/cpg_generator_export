...
10. 新增 provider-based native 自动补源与递归 native 依赖扩展  
   当前已经不再只支持单一组件特判，而是引入了组件 provider 架构，见 [native_source_providers.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/fetch/native_source_providers.py)。  
   当前已支持：
   - 按项目实际 component 自动选择 provider
   - 本地 bundled 源码优先发现
   - 多个常见组件的官方下载候选生成
   - native 源码扫描出的间接依赖扩展
   - `native_analysis_coverage / native_missing_components` 级别的结论降级

11. 新增 Linux 下的严格 native-to-native 解析骨架  
   当前已经实现了三阶段的 Linux strict resolution 主链，核心包括：
   - 二进制与符号发现：`pkg-config / ldconfig / ldd / nm -D`
   - 组件级严格依赖解析：根据 imported/exported symbol 建 `binary-symbol` 级依赖
   - 调用点到导出符号的精确映射：把 `CALL:C` 连到 `EXPORTED_SYMBOL`
   - 链接上下文约束：用 `pkg-config + ldd` 收窄 provider 候选
   - 高置信结论门槛收紧：缺少 callsite-level native 证据时，不再轻易保留 `confirmed`

   对应实现位置：
   - 路径默认值与迁移友好推导： [path_defaults.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/common/path_defaults.py)
   - Linux strict symbol resolver： [native_symbol_resolver.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/fetch/native_symbol_resolver.py)
   - 主流程接入与报告字段： [supplychain_analyze.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/supplychain/supplychain_analyze.py)

### 3.1.1 当前实现成熟度

为了避免论文表述过满，当前实现应区分为三类：

- 已稳定落地
  - Rust 依赖解析、Rust CPG、native 版本探测
  - source-aware degradation
  - 触发条件建模
  - provider-based native 自动补源入口
  - 报告级 `native_analysis_coverage` 降级
  - Linux 下的组件级 strict symbol resolution
  - `BINARY / IMPORTED_SYMBOL / EXPORTED_SYMBOL / RESOLVES_EXTERN_TO` 级别图叠加
  - 关键脚本的迁移友好路径推导与 manifest 环境变量路径展开

- 已落地但仍偏工程雏形
  - 递归 native 依赖扩展
  - 带链接上下文约束的 `NATIVE_DEPENDS_ON / NATIVE_CALL`
  - 大型官方源码下载后的 C CPG 导入
  - callsite-level native 证据对最终 triggerability 的统一校准

- 尚未完全成立
  - 严格、完整、链接器级且覆盖函数指针/callback 的 native-to-native 调用解析
  - 对大型 native 组件稳定且可扩展的全量递归补源分析
  - 仅靠系统自动分析稳定得到大规模 `observable_triggered`
...
- 当前 native 内部分析已经能做到：
  - 导入目标组件的 C CPG
  - 在图中恢复 `METHOD:C / CALL:C / SYMBOL`
  - 验证 symbol、API sequence、部分参数语义
  - 在补入源码后向 native 间接依赖扩展
  - 在 Linux 上根据 `pkg-config / ldconfig / ldd / nm -D` 做 imported/exported symbol 级严格依赖解析
  - 在图上建立 `BINARY / IMPORTED_SYMBOL / EXPORTED_SYMBOL / RESOLVES_EXTERN_TO`
- 但当前对“native 间接依赖”的严格解析仍有边界：
  - 目前 strongest path 是 Linux + shared library + provider 已知组件
  - 当前还没有做到完整链接器级、跨函数指针/callback 的 native-to-native interprocedural call graph reconstruction
  - 对大型组件，严格解析后的全量补源和图导入仍有性能压力

也就是说，当前更准确的表述应是：

> 工具支持 native-internal rule validation，并已实现 Linux 下基于 imported/exported symbol 的严格 native dependency resolution；但目前还没有做到完整、精确、链接器级且覆盖复杂动态调用的 native-to-native 调用图重建。

对应实现位置：

- provider 架构与官方下载候选： [native_source_providers.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/fetch/native_source_providers.py)
- native 自动补源： [native_source_resolver.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/fetch/native_source_resolver.py)
- Linux strict symbol resolver： [native_symbol_resolver.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/fetch/native_symbol_resolver.py)
- 递归 native 扩展： [supplychain_analyze.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/supplychain/supplychain_analyze.py)
- `NATIVE_DEPENDS_ON / NATIVE_CALL / RESOLVES_EXTERN_TO`： [supplychain_analyze.py](/Users/dingyanwen/Desktop/RUST_IR/cpg_generator_export/tools/supplychain/supplychain_analyze.py)
...
- 补源失败或关键 native 间接依赖缺失时，报告中会显式写出：
  - `native_analysis_coverage`
  - `native_dependency_imports`
  - `native_missing_components`
  - `native_dependency_graph_incomplete`
- 当 strict native 解析只能到 `symbol_level`、还没到 `callsite_level` 时，`binary-only/system/stub` 场景下的高置信 `confirmed` 会被降到 `possible`

但这部分仍未完全收紧到论文最理想状态。当前代码仍允许：

- `source_status in {stub, binary-only, system}`
- 且 Rust 侧 trigger 证据足够强时
- 先保留 `possible/confirmed`，再追加降级原因

因此，如果论文想主打“谨慎结论”，后续应继续改成：

- 对必须依赖 native 内部证据的漏洞家族，缺少关键 native 组件源码时，最高只能到 `possible`。
...
1. `Rust -> 目标 native 组件`  
   这一层已经有较强实现，依赖 Rust CPG、FFI 证据、native sink/symbol 和规则化 trigger guard。

2. `目标 native 组件 -> native 间接依赖组件`  
   这一层目前已经进入“严格解析第一版”，具体表现为：
   - 先通过 Linux 下的 imported/exported symbol 做组件级 strict resolution
   - 再通过 `CALL:C -> EXPORTED_SYMBOL` 做 callsite-level overlay
   - 同时保留源码 token/source scan 作为 fallback 证据
   - 再结合 `pkg-config / ldd` 的链接上下文收窄 provider 候选

这意味着当前系统已经能做“递归 native 依赖扩展 + Linux strict resolution”，但还不能把这部分表述成“完整精确的 native-to-native interprocedural call graph reconstruction”。
...
2. **把 native 组件“内部可验证规则”接到供应链 exploitability 分析里，这一点很少见。**  
   ...
   但这里的表述必须精确：
   - 对目标组件内部规则验证，这个主张已经基本成立。
   - 对目标组件的 native 间接依赖，当前更准确的说法应是“source-backed recursive dependency expansion”，而不是“完整 native 内部调用图验证”。

3. **把“无 C 源码”和“有 C 源码”的分析语义统一到一个框架里，目前公开工作里不常见。**  
   这部分特别适合写成 source-aware exploitability reasoning。

   当前这一点已经比文档早期版本更强，因为你已经加入了 provider-based native 自动补源与递归扩展，而不只是“有源码就用、没源码就退化”。
...
另外，当前 native 自动补源、严格解析与递归扩展虽然已经实现，但还存在两类实现性风险，论文里必须诚实面对：

- **性能风险**：大型官方源码下载后，C CPG 导入和图查询代价较高，可能拖慢整轮分析。
- **精度风险**：当前 strict resolution 已经显著强于早期 source scan，但仍主要覆盖 Linux shared library 主路径；对静态链接、函数指针、callback 和复杂导出/别名情况仍不够完备。
...
- 参数别名和长度推导
- Rust 到 C 参数对齐
- Linux strict resolution 向静态链接、函数指针、callback 扩展
- C body 缺失时的置信度校准

当前这块已经完成的基础设施包括：

- Linux 下 `pkg-config / ldconfig / ldd / nm -D` 驱动的 strict symbol resolution
- `BINARY / IMPORTED_SYMBOL / EXPORTED_SYMBOL / RESOLVES_EXTERN_TO` 图叠加
- 基于 callsite-level native 证据的 triggerability 降级

所以下一步不再是“从 0 开始做 native 严格解析”，而是：

- 扩大 strict resolution 的适用边界
- 将 `callsite_level / symbol_level / target_only` 做成系统化实验变量
- 量化 strict resolution 对误报率和结论强度的影响
